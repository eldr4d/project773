...
def first_order_coherence(dic,tweets):
    ...
    FOH = [np.average(x) for x in summary_vec]
    #foh_min = [np.minimum(x) for x in summary_vec]
    foh_median = [np.median(x) for x in summary_vec]
    foh_std = [np.std(x) for x in summary_vec]
...
def sec_order_coherence(dic,tweets):
    ...
    SOH = [np.average(x) for x in summary_vec]
    #soh_min = [np.minimum(x) for x in summary_vec]
    soh_median = [np.median(x) for x in summary_vec]
    soh_std = [np.std(x) for x in summary_vec]
...
def get_coherence(users_tweets):
    ...
    for key in users_tweets.keys():
        for k in users_tweets[key].keys():
            ...
            coherence[key][k]["median_FOC"] = FOC[0]
            coherence[key][k]["std_FOC"] = FOC[0]

            coherence[key][k]["avg_SOC"] = SOC[0]
            coherence[key][k]["median_SOC"] = SOC[0]
            coherence[key][k]["std_SOC"] = SOC[0]